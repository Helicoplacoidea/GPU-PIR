...
CUDA toolkit 12.x installed and visible in PATH (see Configure CUDA environment below)
...
Ensure the CUDA toolkit is on PATH (see Configure CUDA environment).
...
GPU architecture — controlled by CMAKE_CUDA_ARCHITECTURES (see GPU architecture). Defaults to native (auto-detect).
