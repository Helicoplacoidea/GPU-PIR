...
__global__ void Eval_multi_layer(uint32_t *key, int layer_len, int layer, unsigned char *k, uint128_t *s, uint32_t *t, uint128_t *res_s, uint32_t *res_t)
{
	...
	// 清零共享内存
	// for (int i = tid; i < (1 << layer_len) * 2; i += blockDim.x)
	// {
	// 	shared_s[i] = uint128_t(0, 0);
	// }
	// for (int i = tid; i < (1 << layer_len) * 2; i += blockDim.x)
	// {
	// 	shared_t[i] = 0;
...
__global__ void EVAL_Pack_last_layer_warp_n(unsigned char *k, bool *se, uint128_t *d_db, uint128_t *blocksum_cuda)
{
	__shared__ uint128_t sdata[8 * entry_size];
	...
	bool *se_local = (bool *)(se + batch_id * (1 << n) * sizeof(bool));
...
__global__ void EVAL_Pack_last_layer_warp_once(int n, bool *se, uint128_t *d_db, uint128_t *blocksum_cuda)
{
	// 计算线程索引
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	...
	bool *se_local = (bool *)(se + batch_id * (1 << n) * sizeof(bool)); // 局部 SE 指针
...
__global__ void EVAL_Pack_last_layer_gense(unsigned char *k, uint128_t *s, uint32_t *t, bool *se)
{
	...
	res = s_local[idx + (1 << maxlayer) - 1] ^ finalblock.select(t_local[idx + (1 << maxlayer) - 1]);
	// }
	__syncthreads();

	for (int i = 0; i < 128; i++)
	{
		se_local[idx * 128 + i] = res.get_bit(127 - i);
		// if (idx == 0)
...
extern "C" uint128_t *test_dpf_pir(uint8_t *key, uint128_t *db, int N)
{
	...
	uint128_t *blocksum = (uint128_t *)malloc(N * ((1 << n) / 256) * sizeof(uint128_t) * entry_size);
	...
	cudaMallocManaged(&d_eval_res, N * (1 << maxlayer) * sizeof(uint128_t));
	cudaMalloc(&d_se, N * (1 << n) * sizeof(bool));
	...
	// printf("block_sum\n");
	// for (int i = 0; i < N * ((1 << n) / (256)) * entry_size; i++)
	// {
	// 	if (i == ((1 << n) / (256)) * entry_size)
	// 		printf("2rd:\n");
	// 	blocksum_cuda[i].print_uint128("", blocksum_cuda[i]);
	// }
	...
	if (n - 16 >= 0)
	{
		cal_sum_warp<<<N * blocks_per_grid / threads_per_block, threads_per_block>>>(blocksum_cuda, d_k_res);
		cudaDeviceSynchronize();
		cudaMemcpy(k_res, d_k_res, N * ((1 << n) / 65536) * sizeof(uint128_t) * entry_size, cudaMemcpyDeviceToHost);

		for (int m = 0; m < N; m++)
			for (int i = 0; i < (1 << n) / 65536; i++)
			{
				for (int j = 0; j < entry_size; j++)
					res[m * entry_size + j] ^= k_res[m * ((1 << n) / 65536) * entry_size + i * entry_size + j];
			}
	}
...
extern "C" uint128_t *test_dpf_pir_pipeline(uint8_t *key, uint128_t *db, int N)
{
	...
	cudaMalloc(&d_se, (1 << n) * sizeof(bool));
	...
	for (int chunk = 0; chunk < N; chunk++)
	{
		...
		{
			if (chunk > 0)
				cudaStreamWaitEvent(streams[1], events[chunk - 1][maxlayer + 1], 0);
			cudaMemcpyAsync(d_s, &key_pinned[1 + chunk * (1 + 16 + 1 + 18 * maxlayer + 16)], 16, cudaMemcpyHostToDevice, streams[1]);
			cudaMemcpyAsync(d_t, &key_pinned[17 + chunk * (1 + 16 + 1 + 18 * maxlayer + 16)], 1, cudaMemcpyHostToDevice, streams[1]);
		...
		{
			if (chunk > 0)
				cudaStreamWaitEvent(streams[1], events[chunk - 1][maxlayer], 0);
			cudaMemcpyAsync(d_s, &key_pinned[1 + chunk * (1 + 16 + 1 + 18 * maxlayer + 16)], 16, cudaMemcpyHostToDevice, streams[1]);
			cudaMemcpyAsync(d_t, &key_pinned[17 + chunk * (1 + 16 + 1 + 18 * maxlayer + 16)], 1, cudaMemcpyHostToDevice, streams[1]);
			cudaMemcpyAsync(d_key_offset, &key_pinned[chunk * (1 + 16 + 1 + 18 * maxlayer + 16)], (1 + 16 + 1 + 18 * maxlayer + 16) * sizeof(uint8_t), cudaMemcpyHostToDevice, streams[1]);
			...
			if (chunk > 0)
				cudaStreamWaitEvent(streams[maxlayer], events[chunk - 1][maxlayer + 1], 0);
			cudaStreamWaitEvent(streams[maxlayer], events[chunk][1], 0);
			Eval_multi_layer<<<(1 << (maxlayer - 10)), (1 << 10), 0, streams[maxlayer]>>>(aes_key, 10, maxlayer - 10, d_key_offset, d_s + (1 << (maxlayer - 10) - 1), d_t + (1 << (maxlayer - 10) - 1), d_s + (1 << maxlayer) - 1, d_t + (1 << maxlayer) - 1);
			cudaEventRecord(events[chunk][maxlayer], streams[maxlayer]);
		}
		threads_per_block = min(1 << maxlayer, 256);
		blocks_per_grid = (1 << maxlayer) / threads_per_block;

		if (chunk > 0)
			cudaStreamWaitEvent(streams[maxlayer + 1], events[chunk - 1][maxlayer + 5], 0);
		if (maxlayer <= 10)
			cudaStreamWaitEvent(streams[maxlayer + 1], events[chunk][1], 0);
		else if (maxlayer > 10)
			cudaStreamWaitEvent(streams[maxlayer + 1], events[chunk][maxlayer], 0);
		EVAL_Pack_last_layer_gense<<<blocks_per_grid, threads_per_block, 0, streams[maxlayer + 1]>>>(d_key_offset, d_s, d_t, d_se);
		cudaEventRecord(events[chunk][maxlayer + 1], streams[maxlayer + 1]);
		// cudaStreamSynchronize(streams[maxlayer + 1]);
		...
		blocks_per_grid = (1 << n) / threads_per_block;
		if (chunk > 0)
			cudaStreamWaitEvent(streams[maxlayer + 5], events[chunk - 1][maxlayer + 3], 0);

		cudaStreamWaitEvent(streams[maxlayer + 5], events[chunk][maxlayer + 1], 0);
		if (n > 16)
			EVAL_Pack_last_layer_warp_once<<<blocks_per_grid, threads_per_block, 0, streams[maxlayer + 5]>>>(n, d_se, d_db, d_input);
		else if (n <= 16)
			EVAL_Pack_last_layer_warp_n<<<blocks_per_grid, threads_per_block, 0, streams[maxlayer + 5]>>>(d_key_offset, d_se, d_db, d_intermediate4);
		cudaEventRecord(events[chunk][maxlayer + 5], streams[maxlayer + 5]);

		if (n <= 16)
		{

			cudaStreamWaitEvent(streams[maxlayer + 3], events[chunk][maxlayer + 5], 0);
			if (chunk > 0)
				cudaStreamWaitEvent(streams[maxlayer + 3], events[chunk - 1][maxlayer + 4], 0);
			cal_sum_warp<<<1, ((1 << n) + 255) / 256, 0, streams[maxlayer + 3]>>>(d_intermediate4, d_output);
			cudaEventRecord(events[chunk][maxlayer + 3], streams[maxlayer + 3]);
			cudaMemcpyAsync(res + entry_size * chunk * 1, d_output, entry_size * 1 * sizeof(uint128_t), cudaMemcpyDeviceToHost, streams[maxlayer + 3]);
		}
		else if (n > 16 && n <= 20)
		{
			...
			if (chunk > 0)
				cudaStreamWaitEvent(streams[maxlayer + 3], events[chunk - 1][maxlayer + 4], 0);
			cal_sum_warp<<<(CHUNK_SIZE + 255) / 256, 256, 0, streams[maxlayer + 3]>>>(d_input, d_intermediate8);
			cudaEventRecord(events[chunk][maxlayer + 3], streams[maxlayer + 3]);

			cudaStreamWaitEvent(streams[maxlayer + 4], events[chunk][maxlayer + 3], 0);
			cal_sum_warp<<<1, (CHUNK_SIZE + 255) / 256, 0, streams[maxlayer + 4]>>>(d_intermediate8, d_output);

			cudaEventRecord(events[chunk][maxlayer + 4], streams[maxlayer + 4]);
			cudaMemcpyAsync(res + entry_size * chunk * 1, d_output, entry_size * 1 * sizeof(uint128_t), cudaMemcpyDeviceToHost, streams[maxlayer + 4]);
		}
		else if (n > 20 && n <= 28)
		{
			cudaStreamWaitEvent(streams[maxlayer + 3], events[chunk][maxlayer + 5], 0);
			...
			cudaStreamWaitEvent(streams[maxlayer + 4], events[chunk][maxlayer + 3], 0);
			if (chunk > 0)
				cudaStreamWaitEvent(streams[maxlayer + 4], events[chunk - 1][maxlayer + 6], 0);
			cal_sum_warp<<<(CHUNK_SIZE + 255) / 65536, 256, 0, streams[maxlayer + 4]>>>(d_intermediate8, d_intermediate16);
			cudaEventRecord(events[chunk][maxlayer + 4], streams[maxlayer + 4]);

			cudaStreamWaitEvent(streams[maxlayer + 6], events[chunk][maxlayer + 4], 0);
			cal_sum_warp<<<1, (CHUNK_SIZE + 255) / 65536, 0, streams[maxlayer + 6]>>>(d_intermediate16, d_output);
			cudaEventRecord(events[chunk][maxlayer + 6], streams[maxlayer + 6]);

			cudaMemcpyAsync(res + entry_size * chunk * 1, d_output, entry_size * 1 * sizeof(uint128_t), cudaMemcpyDeviceToHost, streams[maxlayer + 6]);
		}

		// // 流水线执行
		// cudaStreamWaitEvent(streams[maxlayer + 3], events[chunk][maxlayer + 2], 0);
		// half_reduction_full_stride<<<(CHUNK_SIZE + 255) / 256, 256, 0, streams[maxlayer + 3]>>>(d_input, d_intermediate1);
		// cudaEventRecord(events[chunk][maxlayer + 3], streams[maxlayer + 3]);

		// // 第二个核函数依赖于第一个核函数的输出
		// cudaStreamWaitEvent(streams[maxlayer + 4], events[chunk][maxlayer + 3], 0);
		// half_reduction_full_stride<<<(CHUNK_SIZE / 2 + 255) / 256, 256, 0, streams[maxlayer + 4]>>>(d_intermediate1, d_intermediate2);
		// cudaEventRecord(events[chunk][maxlayer + 4], streams[maxlayer + 4]);

		// // 第三个核函数依赖于第二个核函数的输出
		// cudaStreamWaitEvent(streams[maxlayer + 5], events[chunk][maxlayer + 4], 0);
		// half_reduction_full_stride<<<(CHUNK_SIZE / 4 + 255) / 256, 256, 0, streams[maxlayer + 5]>>>(d_intermediate2, d_intermediate3);
		// cudaEventRecord(events[chunk][maxlayer + 5], streams[maxlayer + 5]);

		// // 第四个核函数依赖于第三个核函数的输出
		// cudaStreamWaitEvent(streams[maxlayer + 6], events[chunk][maxlayer + 5], 0);
		// half_reduction_full_stride<<<(CHUNK_SIZE / 8 + 255) / 256, 256, 0, streams[maxlayer + 6]>>>(d_intermediate3, d_intermediate4);
		// cudaEventRecord(events[chunk][maxlayer + 6], streams[maxlayer + 6]);

		// // 第五个核函数依赖于第四个核函数的输出
		// cudaStreamWaitEvent(streams[maxlayer + 7], events[chunk][maxlayer + 6], 0);
